...
def compute_sigma(ntod, simtype):
    """
    Compute the white noise sigma for a given number of interferograms, without allocating the
    (potentially huge) noise array itself. Useful when noise needs to be generated in chunks.

    Parameters
    ----------
    ntod : int
        Total number of interferograms (used for the fossil sqrt(ntod) scaling).
    simtype : str
        Type of simulation, e.g. "fossil" or "firas".
    Returns
    -------
    sigma_Mjy : float
        Noise sigma in MJy/sr.
    """
    if simtype == "fossil":
...
def white_noise(ntod, simtype):
    ...
    """
    sigma_Mjy = compute_sigma(ntod, simtype)
